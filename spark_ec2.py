...
def parse_args():
  parser = argparse.ArgumentParser(usage="spark-ec2 [options] <action> <cluster_name>"
      + "\n\n<action> can be: launch, destroy, login, stop, start, get-master",
      add_help=False)
  parser.add_argument("-h", "--help", action="help",
                    help="Show this help message and exit")
  parser.add_argument("-s", "--slaves", type=int, default=1,
  ...
  parser.add_argument("-m", "--master-instance-type", default="",
      help="Master instance type (leave empty for same as instance-type)")
  parser.add_argument("-r", "--region", default="us-west-1",
      help="EC2 region zone to launch instances in")
  parser.add_argument("-z", "--zone", default="",
      help="Availability zone to launch instances in, or 'all' to spread " +
           "slaves across multiple (an additional $0.01/Gb for bandwidth" +
           "between zones applies)")
  parser.add_argument("-a", "--ami", help="Amazon Machine Image ID to use")
  parser.add_argument("-v", "--spark-version", default="0.8.0",
      help="Version of Spark to use: 'X.Y.Z' or a specific git hash")
  parser.add_argument("--spark-git-repo", 
      default="https://github.com/mesos/spark", 
      help="Github repo from which to checkout supplied commit hash")
  parser.add_argument("--hadoop-major-version", default="1",
      help="Major version of Hadoop (default: 1)")
  parser.add_argument("-D", metavar="[ADDRESS:]PORT", dest="proxy_port", 
      help="Use SSH dynamic port forwarding to create a SOCKS proxy at " +
            "the given local address (for use with login)")
  parser.add_argument("--resume", action="store_true", default=False,
      help="Resume installation on a previously launched cluster " +
           "(for debugging)")
  parser.add_argument("--ebs-vol-size", metavar="SIZE", type=int, default=0,
      help="Attach a new EBS volume of size SIZE (in GB) to each node as " +
           "/vol. The volumes will be deleted when the instances terminate. " +
           "Only possible on EBS-backed AMIs.")
  parser.add_argument("--swap", metavar="SWAP", type=int, default=1024,
      help="Swap space to set up per node, in MB (default: 1024)")
  parser.add_argument("--spot-price", metavar="PRICE", type=float,
      help="If specified, launch slaves as spot instances with the given " +
            "maximum price (in dollars)")
  parser.add_argument("--ganglia", action="store_true", default=True,
      help="Setup Ganglia monitoring on cluster (default: on). NOTE: " +
           "the Ganglia page will be publicly accessible")
  parser.add_argument("--no-ganglia", action="store_false", dest="ganglia",
  ...
  parser.add_argument("--master-ip", 
      help="Associate fixed private IP to master")
  parser.add_argument("--naming-prefix", default="data.", 
      help="Prefix your instances names")
  parser.add_argument("--vpc-id",  help="Your VPC id")
  parser.add_argument("--subnet-id", help="Your VPC subnet id")
  parser.add_argument("--access-list", default="0.0.0.0/32", nargs="+", type=str,
    help="List of CIDR can access web UIs and SSH") 
  parser.add_argument("action", default=None, choices="launch,start,stop,destroy,login,get-master",
      help="Action to take on cluster") 
  parser.add_argument("cluster_name", help="Name your cluster")   
  ...
  (action, cluster_name) = (opts.action,opts.cluster_name)
...
def get_or_make_group(conn, name,opts):
  ...
  if len(group) > 0:
    ...
    return conn.create_security_group(name, "Spark EC2 group",vpc_id=opts.vpc_id)
...
def launch_cluster(conn, opts, cluster_name):
  ...
  if master_group.rules == []: # Group was just now created
    ...
    for cidr in opts.access_list:
      ...
      master_group.authorize('tcp', 4040, 4045, cidr)
      master_group.authorize('tcp', 3030, 3030, cidr)
      if opts.ganglia:
        master_group.authorize('tcp', 5080, 5080, cidr)
  if slave_group.rules == []: # Group was just now created
    ...
    for cidr in opts.access_list:
      ...
      slave_group.authorize('tcp', 50060, 50060, cidr)
  ...
  slave_interface = boto.ec2.networkinterface.NetworkInterfaceSpecification(subnet_id=opts.subnet_id,
                                                                            groups=[slave_group.id],
                                                                            associate_public_ip_address=True)

  slave_interfaces = boto.ec2.networkinterface.NetworkInterfaceCollection(slave_interface)
  ...
  if opts.spot_price != None:
    ...
    for zone in zones:
      ...
      slave_reqs = conn.request_spot_instances(
          ...
          network_interfaces=slave_interfaces,
  ...
  master_res = conn.run_instances(image_id=image.id,
                    ...
                    network_interfaces=master_interfaces,
                    key_name=opts.key_pair,
                    min_count = 1,
                    max_count = 1,
                    block_device_map = block_map)
  ...
  for i in master_nodes:
    i.add_tag('Name','data.'+ cluster_name + '.master' + str(j))
    j = j+1
  j=1  
  for i in slave_nodes:
    i.add_tag('Name','data.'+ cluster_name + '.slave' + str(j))
    j = j+1
...
def get_existing_cluster(conn, opts, cluster_name, die_on_error=True):
  ...
  for res in reservations:
    ...
    if len(active) > 0:
      for i in active:
        if 'Name' in i.tags and i.tags[u'Name'].startswith(opts.naming_prefix + cluster_name + ".master"):
          master_nodes += [i]  
        if 'Name' in i.tags and i.tags[u'Name'].startswith(opts.naming_prefix + cluster_name + ".slave"):
          slave_nodes += [i]
...
def setup_cluster(conn, master_nodes, slave_nodes, opts, deploy_ssh_key):
  ...
  ssh(master, opts, "rm -rf spark-ec2 && git clone https://github.com/lelayf/spark-ec2.git -b rootified")
...
def setup_standalone_cluster(master, slave_nodes, opts):
  ...
  if opts.user == "root" :
    ...
    ssh(master, opts, "/home/"+opts.user+"/spark/bin/start-all.sh")
...
def deploy_files(conn, root_dir, opts, master_nodes, slave_nodes, modules):
  ...
  template_vars = {
    ...
    "master_private_fqdn_list": ' '.join([i.private_dns_name for i in master_nodes]),
    "master_private_ip_list": ' '.join([i.private_ip_address for i in master_nodes]),
    "active_master": active_master,
    "slave_list": ' '.join([i.public_dns_name for i in slave_nodes]),
    "slave_private_fqdn_list": ' '.join([i.private_dns_name for i in slave_nodes]),
    "slave_private_ip_list": ' '.join([i.private_ip_address for i in slave_nodes]),
  ...
  if opts.user == "root":
    command = (("rsync -rv -e 'ssh -o StrictHostKeyChecking=no -i %s' " +
        "'%s/' '%s@%s:/root'") % (opts.identity_file, tmp_dir, opts.user, active_master))
    subprocess.check_call(command, shell=True)
  else:
    command = (("rsync -rv -e 'ssh -o StrictHostKeyChecking=no -i %s' " +
        "'%s/' '%s@%s:/home/"+opts.user+"'") % (opts.identity_file, tmp_dir, opts.user, active_master))
    subprocess.check_call(command, shell=True)    
...
def main():
  ...
  try:
    ...
    conn = boto.vpc.connect_to_region(opts.region)
